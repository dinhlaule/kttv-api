package com.neo.kttvapi.dto.mapper;
...
public abstract class Mapper {
    ...
    public static <S, T> Set<T> toSet(List<S> source, Class<T> targetClass) {
        ModelMapper modelMapper = new ModelMapper();
        modelMapper.getConfiguration().setAmbiguityIgnored(true);
        return source.stream().map(item -> modelMapper.map(item, targetClass)).collect(Collectors.toSet());
    }
