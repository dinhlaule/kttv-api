...
public class ApiExceptionHandler {
	...
	public @ResponseBody
	ResponseEntity<ResponseBodyDto<Object>> handleBindException(BindException ex, WebRequest request) {
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		ResponseBodyDto<Object> dtoResult = new ResponseBodyDto<>();
		dtoResult.setCode(ResponseCodeEnum.R_400);
		dtoResult.setMessage(ex.getBindingResult().getAllErrors().get(0).getDefaultMessage());
